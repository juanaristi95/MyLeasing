...
namespace MyLeasing.Web.Data
{
    ...
    {
        /*
        Notea: El seedDB va a meter datos a la BD por lo que necesita 
        instanciarla, por lo que aplicaremos la inyeccion de dependencias.
        
        A los atributos privados de una clase ya no se les coloca this. sino _
        */

        private readonly DataContext _context;
        //le decimos que al atributo privados le llevamos la inyeccion
        ...
        {
            // garantiza que cuando pase por aqui la BD este creada.
            await _context.Database.EnsureCreatedAsync();
            // crea los tipos de propiedades
        ...
        {
            // si no hay registros inserte
            if (!_context.PropertyTypes.Any())
            {
                // añadimo registro a la tabla
                ...
                // salvo los registros insertado - es donde de verdad se inserta los reg.
        ...
        {
            // para crear una propiedad necesitamos un Owner y una Property
            // si no lo hacemos de esta forma, las relaciones entre las tablas
            // no nos permitiran realizar la insercion.
            var owner = _context.Owners.FirstOrDefault();
            var propertyType = _context.PropertyTypes.FirstOrDefault();
            if (!_context.Properties.Any())
            {
                // se le adiciona un M al final debido a que es deciaml para
                // que o crea que sea un double
